...
#DesignMatrix,TrainingValues= np.matrix([50, -1, 10,-10,]), np.matrix([ 1, 0, 1, 0]) # if independent variable < 0 then out is 0, if > 0 then 1
...
#DesignMatrix,TestingValues= np.matrix([50, -1, 10, -10]), np.matrix([ 1, 0, 1, 0])
X=np.transpose(DesignMatrix)  # make column vector inputs
#Y=np.transpose(TestingValues)
#X = DesignMatrix
#Y = TestingValues
