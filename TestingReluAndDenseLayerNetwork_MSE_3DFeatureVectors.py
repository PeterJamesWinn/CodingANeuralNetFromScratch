...
DesignMatrix1, DesignMatrix2, DesignMatrix3, TrainingValues= GenerateTrainingData_3DFeature(1,10)
X = np.concatenate((np.transpose(DesignMatrix1), np.transpose(DesignMatrix2), np.transpose(DesignMatrix3)), axis=1)
print("X", X)
...
#Network topology
#network = [dense_layer(3,6), relu_layer(), dense_layer(6,6), relu_layer() , dense_layer(6,6),relu_layer() , dense_layer(6,1)]
...
# Test network.
# using training data
X_Test = np.concatenate((np.transpose(DesignMatrix1), np.transpose(DesignMatrix2), np.transpose(DesignMatrix3)), axis=1)
print("X", X)
Y=np.transpose(TrainingValues)
...
DesignMatrix1, DesignMatrix2, DesignMatrix3, TestingValues = GenerateTrainingData_3DFeature(40,50)
X_test = np.concatenate((np.transpose(DesignMatrix1), np.transpose(DesignMatrix2), np.transpose(DesignMatrix3)), axis=1)
print("X", X)
